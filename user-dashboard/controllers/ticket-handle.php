...
elseif(!isset($_POST['sb'])){
    ...
    if(empty($msg) || empty($user)){
        header("Location: ../?support&error=emptyField");
        exit();
    }
    $msg = nl2br($msg);
    ...
    if(empty($_FILES['attachment']['name'])){
        ...
        if(!mysqli_stmt_prepare($stmt , $sql)){
            ...
            mysqli_stmt_bind_param($stmt , "sssss" , $user , $msg , $from_status , $date,$ns);
            mysqli_stmt_execute($stmt);
            $short_msg = "شما یک پیغام از <strong>$username</strong> دریافت کردید.";
            $msg = "شما یک پیغام از <strong>$username</strong> دریافت کردید.";
            $nf = "سیستم";
            $nt = "admins";
            $ns = "unseen";
        ...
        if(!mysqli_stmt_prepare($stmt , $sql)){
            ...
            mysqli_stmt_bind_param($stmt , "ssssss" , $user , $msg , $from_status , $date , $file_name,$ns);
            mysqli_stmt_execute($stmt);
